...
## IMPORTANT: Codebase Intelligence Instructions for prview
...
Last indexed: 2026-06-30 (commit fbaab96)
### Entry Points
- `prview/__main__.py`
- `prview/server.py`
- `prview/static/app.js`
### Tech Stack
**Languages:** Python
...
|------|-------|-------------|-------|
| `prview/static/app.js` | 96.6th %ile | 16 | lefv |
| `prview/repowise.py` | 93.1th %ile | 10 | gwicho38 |
| `tests/test_repowise_core.py` | 89.7th %ile | 10 | gwicho38 |
| `prview/server.py` | 86.2th %ile | 10 | lefv |
| `tests/test_repowise_api.py` | 82.8th %ile | 6 | gwicho38 |
...
Defect risk, Hotspot health: 3.28/10 (stable) ·
Average: 7.02/10 ·
Worst: 1.0/10 (`prview/static/app.js`)
Maintainability, Average: 8.37/10
Performance risk, Average: 9.71/10

### Critical biomarkers
- `prview/api_models.py` — hidden coupling — impact −1.8
- `tests/test_repowise_core.py` — prior defect — impact −1.5
- `prview/server.py` — hidden coupling — impact −1.5
- `prview/repowise.py` — prior defect — impact −0.6
- `prview/static/app.js` — brain method (renderRwError) — impact −0.2
